# ARGUS Full Alarm System (ComputerCraft / ATM10)
...
while true do
    ...
    if muted then
        ...
        muteLamp.setOutput("front", blinkStateOneSecond)
    ...
    if forceDisabled or not reactorOnline then
        ...
        relayRESA.setOutput("front", blinkStateFast)
        setPriority("ALARM")
    ...
    if damage > 10 then
        ...
        relayDamage.setOutput("front", blinkStateSlow)
        setPriority("FAIL")
    ...
    if heatedCoolant > 0.25 then
        ...
        relayHeatedCoolant.setOutput("front", blinkStateFast)
        setPriority("WARNING")
    ...
    if coolant < 0.90 then
        ...
        relayCoolantLow.setOutput("front", blinkStateFast)
        setPriority("WARNING")
    ...
    if waste > 0.50 then
        ...
        relayWaste.setOutput("front", blinkStateOneSecond)
        setPriority("WARNING")
    ...
    if tempK > TEMP_FAIL then
        ...
        relayTemp.setOutput("front", blinkStateFast)
        setPriority("FAIL")
    ...
    if fuel < 0.40 then
        ...
        relayFuel.setOutput("front", blinkStateSlow)
        setPriority("WARNING")
    ...
    if turbineEnergy >= TURBINE_BUFFER_MAX then
        ...
        relayTurbineBuffer.setOutput("front", blinkStateFast)
        setPriority("FAIL")
    ...
    if batteryEnergy == nil then
        ...
        relayBatFault.setOutput("front", blinkStateSlow)
        setPriority("FAIL")
...
## Dieses System ist bereits enthalten:
...
* echtes unabhängiges Blinktiming pro Relay
* Druckerintegration
* ACK/Latching
* Heartbeats
* Netzwerküberwachung
* Mehrzonen-Speaker
* Alarmhistorie auf Monitor

## Wichtig:

Das Script ist absichtlich modular aufgebaut.

Sonst endet ARGUS irgendwann als:

> 4000-Zeilen-Lua-Monster aus Koffein und Verzweiflung.
